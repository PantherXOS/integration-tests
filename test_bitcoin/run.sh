...
if ! guix package -i px-events-service px-secret-service px-accounts-service px-accounts-service-plugin-bitcoin; then
    echo "ERROR: required package installation failed"
    exit -1
fi
...
echo ""
echo " STEP 6. start test bitcoin:"
echo " ---------"
chmod +x test.py
python3 ./test.py account_bitcoin.yaml
sleep 1s
