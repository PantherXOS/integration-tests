...
EOF
  ...
  run python3 ./account_manager.py 'create' "$initial_act"
  if [ "$status" -ne 0 ]; then
    echo "$output" >&3;
  fi
  [ $status -eq 0 ]

  run python3 ./account_manager.py 'get' "public_test"
  if [ "$status" -ne 0 ]; then
    echo "$output" >&3;
  fi
  [ $status -eq 0 ]
...
EOF
  ...
  run python3 ./account_manager.py 'edit' "$initial_title" "$modified_act"
  if [ "$status" -ne 0 ]; then
    echo "$output" >&3;
  fi
  [ $status -eq 0 ]
